...
def main():
    ...
    for utt_id, (audio, fs) in tqdm(dataset):
        ...
        # resample audio if sr different from config -- added by Ashi not in original code so caution
        if fs != config["sampling_rate"]:
            audio = librosa.resample(audio, orig_sr=fs, target_sr=config["sampling_rate"])
            print(f"resampled {utt_id} from {fs} to {config["sampling_rate"]}")
        # trim silence
        ...
        # apply global gain
        # print(config["global_gain_scale"])
        # Added by Ashi
        # if np.abs(audio).max() >= 1.0:
        #     audio = torch.from_numpy(audio)
        #     print('Clamping the audio')
        #     audio = torch.clamp(audio,-1,1)
        #     audio = audio.numpy()
        #     print(f"After clamping: max = {audio.max()}, min = {audio.min()}")

        print(np.abs(audio).max(), np.abs(audio).min())
        ...
        if np.abs(audio).max() >= 1.0:
            ...
            # continue; ignoring the clipping part here -- Ashi?
